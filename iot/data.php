<?php
    ...
    switch($_GET['q']){
        ...
        break;
        case 2:
            $statement=$pdo->prepare("SELECT `timeStamp` as x, temperature as y FROM (SELECT * FROM `templog` ORDER BY `timeStamp` DESC LIMIT 0,80)`templog` ORDER BY `timeStamp` ASC");
            $statement->execute();
            $results=$statement->fetchAll(PDO::FETCH_ASSOC);
            $json=json_encode($results);
            echo $json;
        ...
        break;
        case 6:
            $statement=$pdo->prepare("SELECT `timeStamp` as x, humidity as y FROM (SELECT * FROM `templog` ORDER BY `timeStamp` DESC LIMIT 0,80)`templog` ORDER BY `timeStamp` ASC");
            $statement->execute();
            $results=$statement->fetchAll(PDO::FETCH_ASSOC);
            $json=json_encode($results);
            echo $json;
        ...
        break;
        case 8:
            $statement=$pdo->prepare("SELECT `timeStamp` as x, presser as y FROM (SELECT * FROM `templog` ORDER BY `timeStamp` DESC LIMIT 0,80)`templog` ORDER BY `timeStamp` ASC");
            $statement->execute();
            $results=$statement->fetchAll(PDO::FETCH_ASSOC);
            $json=json_encode($results);
            echo $json;
        ...
        break;
        case 10:
            $statement=$pdo->prepare("SELECT `timeStamp` as x, soiltemperature as y FROM (SELECT * FROM `templog` ORDER BY `timeStamp` DESC LIMIT 0,80)`templog` ORDER BY `timeStamp` ASC");
            $statement->execute();
            $results=$statement->fetchAll(PDO::FETCH_ASSOC);
            $json=json_encode($results);
            echo $json;
